...
@Component({
    selector: 'app-appointmentVendedor',
    templateUrl: './appointmentVendedor.component.html',
    styleUrls: ['./appointmentVendedor.component.scss']
})
export class AppointmentVendedorComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(
        public dialogRef: MatDialogRef<AppointmentVendedorComponent>,
    ...
    useAppPAtient(){
        return this.coreService.currentUser.app_patient.id
    }
    ...
    save() {
        ...
        const app_user_id =  `app_user_id: ${this.useAppPAtient()}`;
        ...
        const vendedor = this.data.appointment ? `vendedor: "${this.data.appointment.vendedor}",` : `vendedor: "${this.coreService.currentUser.app_patient.name}",`;
        const conductor = this.data.appointment ? `conductor: "${this.data.appointment.conductor}",` : `conductor: "${data.conductor.name}",`;
        const queryParams = ` ${id}  ${name} ${metros} ${type_concreto} ${date} ${time} ${end_time} ${app_user_id} ${user_id} ${patient_id} ${status} ${direccion} ${vendedor} ${conductor} ${observaciones} ${tipo_descarga}`;
        const queryProps = 'id, user_id, conductor, tipo_descarga, observaciones';

        this.apiService.createAppointment(queryParams, queryProps).subscribe(
            (response: any) => {
                this.sending = false;
