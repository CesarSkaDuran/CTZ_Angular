...
@Component({
    ...
    styleUrls: ['./vendedor-modal.component.scss']
})
export class VendedorModalComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    constructor(
        public dialogRef: MatDialogRef<VendedorModalComponent>,
    ...
    save() {
      ...
      //  const patient_id = this.data.appointment ? `patient_id: ${this.data.appointment.patient_id},` : `patient_id: ${data.cliente.id}`;
       ...
       // const name = `name: "${data.cliente.name}",`;
        ...
        const queryParams = ` ${id}  ${vendedor} `;
        const queryProps = 'id, vendedor';
