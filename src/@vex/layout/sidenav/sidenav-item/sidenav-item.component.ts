...
export class SidenavItemComponent implements OnInit, OnChanges, OnDestroy {
  ...
  constructor(private router: Router,
              ...
              private navigationService: NavigationService, private coreService: CoreService) { }
  ...
  ngOnInit() {
    console.log("Cargados los items sidenav");
    console.log(this.coreService.currentUser.role_id);
  ...
  hiddeItem(label){
    if((this.rol_usuario != 1 && this.rol_usuario != 5) && label =='Pagos'){
      return true;
    }

    if((this.rol_usuario == 8 || this.rol_usuario == 7) && label =='Programación'){
      return true;
    }

    if((this.rol_usuario == 8 || this.rol_usuario == 7) && label =='Calendario'){
      return true;
    }

    if(this.rol_usuario == 8 && label =='Despachos'){
      return true;
    }

    if(this.rol_usuario != 1 && label =='Configuraciones'){
      return true;
    }

    if(this.rol_usuario != 1 && label =='Aprobacion'){
      return true;
    }

    if(this.rol_usuario == 8 && label =='Clientes'){
      return true;
    }

    
    if(this.rol_usuario == 8 && label =='Tipo de concreto'){
      return true;
    }

        
    if(this.rol_usuario == 8 && label =='Conductores'){
      return true;
    }

    if(this.rol_usuario == 8 && label =='Vendedores'){
      return true;
    }

    if(this.rol_usuario == 8 && label =='General'){
      return true;
    }

    if(this.rol_usuario != 8 && label =='Panel'){
      return true;
    }
